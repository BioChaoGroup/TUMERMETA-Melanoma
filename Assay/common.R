...
calPCA <- function(prf,pr,phe,id){
  ...
  pc.score <- as.data.frame(pc$x[,2:3])

  pc.loading <- as.data.frame(pc$rotation[,1:2]) # 1:2 表示pca1:pca2; 2:3代表pc2:pc3
...
speAb_top10 <- function(dat, Mlesstaxa){
  ...
  if (linnum >= dim(dat)[1]-10){
    ...
    others <- rbind(dat[which(rownames(dat) == Mlesstaxa),], others) 
    others <- colSums(others) #
    dat <- dat[-which(rownames(dat) == Mlesstaxa), ]
    dat <- rbind(others, dat[(dim(dat)[1]-9):dim(dat)[1],])
  }
  ...
  if (linnum < dim(dat)[1]-10){
    ...
    dat <- rbind(dat[others,dim(dat)[1]-10:-1,])
  }
  names <- rownames(dat)
  rownames(dat) <- NULL
  names[1] <- "other"
...
speAb_top20 <- function(dat, Mlesstaxa){
  dat$sum <- rowSums(dat) # 添加一列为sum 为每行的丰度和
  dat <- dat[order(dat$sum),] # 根据每行丰度和从小到大排序
  dat <- dat[,-which(colnames(dat) == "sum")]
  linnum <- which(rownames(dat) == Mlesstaxa)  # 找到行头为“Other”的行号
  ...
  # 若others的丰度为前十，则保留丰度为11的菌，其他相加
  if (linnum >= dim(dat)[1]-20){
    for (j in 1:dim(dat)[2]){
      others[j] <- sum(dat[1:(dim(dat)[1]-21),j])
    }
    others <- rbind(dat[which(rownames(dat) == Mlesstaxa),], others) 
    others <- colSums(others) #
    dat <- dat[-which(rownames(dat) == Mlesstaxa), ]
    dat <- rbind(others, dat[(dim(dat)[1]-19):dim(dat)[1],])
  }

  # 若others的丰度不为前十，则其他相加
  if (linnum < dim(dat)[1]-20){
    for (j in 1:dim(dat)[2]){
      others[j] <- sum(dat[1:(dim(dat)[1]-20),j])
    }
    dat <- rbind(dat[others,dim(dat)[1]-20:-1,])
  }
  names <- rownames(dat)
  rownames(dat) <- NULL
  names[1] <- "other"
